...
// Simple μ-law (Twilio) ↔ 16-bit PCM (xAI) converter
...
function pcmToUlaw(pcmBuffer) {
  // Twilio Media Streams expects μ-law - for simplicity we send raw PCM and let Twilio handle it
  return pcmBuffer;
}
...
router.ws('/stream', (ws, req) => {
  ...
  xaiWs.on('open', () => {
    xaiWs.send(JSON.stringify({
      ...
      session: {
        ...
        tools: [
          // ← YOUR EXISTING TOOLS GO HERE (copy them from the console if needed)
          // For now I'm leaving a placeholder - replace with your full list
          {
            type: "function",
            name: "lookup_account",
          ...
          // ← NEW: Milo can now search his own .md files (MEMORY.md, IDENTITY, etc.)
          {
            "type": "file_search",
  ...
  xaiWs.on('message', (data) => {
    ...
    if (event.type === 'response.function_call_arguments.done') {
      // TODO: Implement your tool handlers here (lookup_account, etc.)
...
// Optional: Call status webhook (for logging / Gorgias ticket creation)
router.post('/status', (req, res) => {
  console.log('📞 Call ended', req.body);
  // You can add Gorgias ticket creation here later
  res.sendStatus(200);
});
